...
def disp_mymap()
    @mapsize_y.times do |my|
        @mapsize_x.times do |mx|
            ...
            printf("%1s%1d " , @chara[@mymap[my][mx].to_i][0],@chmap[my][mx]%10)
        end
        es_reset
        printf("\n")
    end
...
loop do # 無限ループ
    ...
    if  rstr.index(M_CHARA.to_s) then # 敵がいる！
        ...
        # 敵がいなかったら@chmap見てsaveしてあるところをブロックにする処理
        (1..9).each do |i|
    ...
    if queue.length == 0 && !estr.include?(M_C[M_ITEM][0]) && !estr.include?(M_C[M_CHARA][0]) then
        mode = D_UP
        queue = find_route(x,y,M_ITEM)
    end
    if queue.length == 0 && !estr.include?(M_C[M_ITEM][0]) && !estr.include?(M_C[M_CHARA][0]) then
        mode = D_UP
        queue = find_route(x,y,M_UNKNOWN)
